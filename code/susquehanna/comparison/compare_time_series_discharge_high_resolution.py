...
iCase_index_e3sm_unstructurd = 6
aParameter_case = pye3sm_read_case_configuration_file(sFilename_case_configuration,                                                        
                                                          iFlag_atm_in = 0,
                                                          iFlag_datm_in = 1,
                                                          iFlag_lnd_in= 0,
...
for sKey, aValue in pDatasets_parameter.variables.items():            
    if (sKey.lower() == 'cellid'):                   
        ...
        continue
cell_index = np.where(aData_id == id_unstructured)
aData_unstructured_high = np.full(nDay, np.nan, dtype= float)
for iYear in range(iYear_start, iYear_end + 1):
    sYear = "{:04d}".format(iYear) #str(iYear).zfill(4)            
    sDate = sYear + '*'    
    sPattern = '*.mosart.h1.' + sDate + sExtension_netcdf
    ...
    if iCount ==1 :            
        ...
        for i in range(0,ndays):
            ...
            lIndex = np.where(aDate == dateobj)
            #print(aData_variable[i, cell_index])
            aData_unstructured_high[lIndex] = aData_variable[i, cell_index]
...
sFilename_out = current_file_directory + slash + 'streamflow_comparison_high.png'
plot_time_series_data( [aDate, aDate, aDate, aDate], [aDischarge_obs, aData_structured, aData_unstructured, aData_unstructured_high], 
                      sFilename_out = sFilename_out,
                      iFlag_scientific_notation_in = 1,
                      sLabel_y_in= 'Streamflow (m$^3$/s)',
                      aLabel_legend_in=['Observation','1/16 DRT-based','MPAS-based','MPAS-based (high-res)'],
                        aColor_in=['black','red','blue','green'],
                          aLinestyle_in=['solid','solid','solid','solid'],
                            aMarker_in=['None','None','None', 'None'  ]  )
