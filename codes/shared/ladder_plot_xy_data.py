...
def ladder_plot_xy_data(aX_all,
    ...
    if aLinewidth_in is not None:
        aLinewidth = aLinewidth_in
    else:
        aLinewidth = np.full(nData, 1)
    ...
    if iFlag_twiny_in is not None:
        ax_twin = ax.twinx()
        sLabel_y_twin = 'Water depth (m)'
        #ax_twin.set_ylabel(sLabel_y_twin, fontsize=12)
        color = aColor[1]  # Set your desired color here
        # Set the y-axis label color
        ax_twin.set_ylabel(sLabel_y_twin, fontsize=12, color=color)
        # Set the y-axis tick parameters color
        ax_twin.tick_params(axis='y', colors=color)
        # Set the y-axis spine color
    ...
    for i in np.arange(1, nData+1):
        if iFlag_twiny_in is None:
            ...
            y1 = aY_all[i-1]
            ax.step(x1, y1,
                    color=aColor[0], linestyle=aLinestyle[i -1],
                      linewidth=aLinewidth[i-1],
                    #marker=aMarker[i-1],
                    #markersize=6,
                    label=aLabel_legend[i-1])

            y3 = aY_all_twin_in[i-1]
            ax_twin.step(x1, y3,
                    color=aColor[1], linestyle=aLinestyle[i -1],
                      linewidth=aLinewidth[i-1],
                    #marker=aMarker[i-1],
                    #markersize=4,
                    label=aLabel_legend_twin_in[i-1])
    ...
    if (iFlag_format_x == 1):
        ax.xaxis.set_major_formatter(mpl.ticker.FormatStrFormatter(sFormat_x))
    if (iFlag_format_y == 1):
    ...
    if iFlag_twiny_in is not None:
        ax_twin.set_ylim(0, dMax_y_twin_in)
    ...
    if iFlag_label == 1:
        ...
        for i in np.arange(1, len(aLabel_point)+1, 1):
            ...
            aText.append(
                ax.text(aX_all[1][i-1], aY_all[1][i-1], sLabel_point, color='red'))
    ...
    if iFlag_twiny_in is not None:
        ...
        handles = handles_ax + handles_twin
