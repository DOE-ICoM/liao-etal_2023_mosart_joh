...
sFilename_domain_geojson = '/compyfs/liao313/04model/e3sm/susquehanna/cases_aux/e3sm20240101002/mosart_susquehanna_parameter_mesh.geojson'
...
sFilename_parameter_in= '/compyfs/liao313/04model/e3sm/susquehanna/cases_aux/e3sm20240101002/mosart_susquehanna_parameter.nc'
...
pDomain = nc.Dataset(sFilename_parameter_structured_in)
...
for sKey in pDomain.variables.keys():
    ...
    if sKey == 'areaTotal2':
        aArea = pDomain.variables[sKey][:]

dDrainage_max0 = np.max(aArea)
...
#find which cell contains the site
sFilename_domain_geojson = '/compyfs/liao313/04model/e3sm/susquehanna/cases_aux/e3sm20240102001/mosart_susquehanna_parameter_mesh.geojson'
pDriver = ogr.GetDriverByName('GeoJSON')
pDataset = pDriver.Open(sFilename_domain_geojson, gdal.GA_ReadOnly)
pLayer = pDataset.GetLayer(0)
pSrs = osr.SpatialReference()
...
dDrainage_max1 = np.max(aArea)/1.0E6
...
sFilename_out = '/qfs/people/liao313/workspace/python/liao-etal_2023_mosart_joh/figures/susquehanna/ladder.geojson'
pDriver = ogr.GetDriverByName('GeoJSON')
...
#print different in ratio
d1 = np.max(aData_structured[np.where(aData_structured < 1.0E10)])
d2 = np.max(aData_unstructured[np.where(aData_unstructured < 1.0E10)])
print(  abs(d1-d2) / np.max([d1,d2])   )
...
aColor = ['red', 'blue']
aMarker= ['s', 'h']
aSize = np.full(1, mpl.rcParams['lines.markersize'])
aLinestyle = ['-', '--']
aLinewidth = [1.0,0.75]
...
sFilename_out = '/qfs/people/liao313/workspace/python/liao-etal_2023_mosart_joh/figures/susquehanna/ladderplot_river_discharge.png'
ladder_plot_xy_data(aX_all,  aY_all,
        sFilename_out,
        aY_all_twin_in=aY_all_twin_in, iFlag_twiny_in=1,aLabel_legend_twin_in=['DRT mesh-based water depth', 'MPAS mesh-based water depth'], dMax_y_twin_in=6,
